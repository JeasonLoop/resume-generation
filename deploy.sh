...
wait_for_service() {
    ...
    local retry=0
    local check_url="http://localhost:80/api/health"   # 通过 Nginx 标准 HTTP 端口检查
...
show_access_info() {
    ...
    echo ""
    echo "访问地址:"
    echo "  - 应用地址: http://你的域名或IP"
    echo "  - HTTPS: https://你的域名（需已配置 SSL 且安全组放行 443）"
    echo "  - API地址: http://你的域名或IP/api"
    echo "  - 健康检查: http://你的域名或IP/api/health"
    echo ""
    echo "测试账号:"
